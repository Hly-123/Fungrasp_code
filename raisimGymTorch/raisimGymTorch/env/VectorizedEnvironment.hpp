...
#pragma omp parallel for
 ...
 private:
  ...
  inline void perAgentStep(int agentId,
    ...
    // RSWARN("agentId=");
    // RSWARN(agentId);
    // RSWARN("action.row(agentId)[0]");
    // RSWARN(action.row(agentId)[0]);
    // RSWARN("action.row(agentId)[1]");
    // RSWARN(action.row(agentId)[1]);
    // RSWARN("action.row(agentId)[2]");
    // RSWARN(action.row(agentId)[2]);
    ...
    // if (agentId == 3)
    // {
    //   RSWARN("I have come in 3 iteration!!!!!!!!!!!!!");
    //   RSWARN(done);
    //   RSWARN(reward);
    // }
    // else
    // {
    //   RSWARN("I have come in 3 iterationXXXXXXXXXXXXXX");
    // }

    // ly explain: 好像卡在这里reward[agentId] 【！！】
    
    // ly modified 【！！】
    reward[agentId] = environments_[agentId]->step(action.row(agentId));
    // reward[agentId] = environments_[agentId]->step(action.row(17));

    // RSWARN("【reward[agentId] in PRINT REWARD in VectoredEnv.hpp】");
    // RSWARN(agentId);
    // RSWARN(reward[agentId]);
    ...
    // ly modified
    // LY EXPLAIN: 程序根本不会运行到这里，在reward[agentId] = environments_[agentId]->step(action.row(agentId));就卡住了【！！】
    // if (agentId == 3)
    // {
    //   RSWARN("I have come in 3 iteration!!!!!!!!!!!!!");
    //   RSWARN(done);
    ...
    // else
    // {
    //   RSWARN("I have come in 176 iterationXXXXXXXXXXXXXX");
    // }

    if (done[agentId]) {
      // RSWARN("I have come in & start to RESET!!!!!!!");
      environments_[agentId]->reset();
      reward[agentId] += terminalReward;
      // RSWARN("I have come in & finish DONE[agentId]!!!!!");
      // RSWARN(agentId);
    }
  }
